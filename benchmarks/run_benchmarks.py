...
def build_cases() -> list[tuple[str, str, str, Callable[[], object]]]:
    ...
    iris = iris_dataset.data[:, 0].tolist()
    wine = wine_dataset.data[:, 0].tolist()
    cancer = cancer_dataset.data[:, 0].tolist()

    graph = nx.karate_club_graph()
    unweighted = {node: list(graph.neighbors(node)) for node in graph.nodes}
    weighted = {
        node: [
            (neighbour, float(data.get("weight", 1.0)))
    ...
    text = "|".join(iris_dataset.target_names) * 2_000
    pattern = "versicolor"
    label_pairs = list(zip(iris_dataset.target_names, wine_dataset.target_names))
    ...
    return [
        (
            ...
            "Seeded synthetic",
            "5,000 values",
            lambda: merge_sort(random_values),
        ),
        (
            "LIS dynamic programming",
            "Wine",
            "120 rows",
            lambda: longest_increasing_subsequence_dp(wine[:120]),
        ),
        (
            "LIS patience sorting",
            "Wine",
            "178 rows",
            lambda: longest_increasing_subsequence(wine),
        ),
        (
            "Online mean/variance",
            "Breast Cancer Wisconsin",
            "569 rows",
            lambda: online_mean_variance(cancer),
        ),
        (
            "KMP search",
            "Iris class labels",
            f"{len(text):,} chars",
            lambda: kmp_search(text, pattern),
        ),
        (
            "Z-function",
            "Iris class labels",
            f"{len(text):,} chars",
            lambda: z_function(text),
        ),
        (
            "Edit distance",
            "Iris/Wine labels",
            "3 label pairs",
            lambda: [edit_distance(first, second) for first, second in label_pairs],
        ),
        (
            "Fenwick build and query",
            "Iris",
            "150 rows",
            lambda: FenwickTree(iris).range_sum(0, len(iris)),
        ),
        (
            "Breadth-first path",
            "Karate Club",
            "34 nodes / 78 edges",
            lambda: breadth_first_path(unweighted, 0, 33),
        ),
        (
            "Dijkstra path",
            "Karate Club",
            "34 nodes / 78 edges",
            lambda: dijkstra_shortest_path(weighted, 0, 33),
        ),
        (
            "Kruskal spanning tree",
            "Karate Club",
            "34 nodes / 78 edges",
            lambda: kruskal_minimum_spanning_forest(graph.nodes, edges),
        ),
    ]
...
def markdown(results: list[Result], repeats: int) -> str:
    ...
    rows.extend(
        (
            f"| {result.rank} | {result.benchmark} | {result.dataset} | "
            f"{result.input_size} | {result.median_ms:.4f} | "
            f"{result.relative_speed:.2f}× |"
        )
    ...
    rows += [
        ...
        "",
        (
            "These timings compare different workloads and therefore rank execution "
            "cost, not algorithmic quality."
        ),
        (
            "For like-for-like comparison, focus on the traditional O(n²) LIS versus "
            "the O(n log n) patience-sorting implementation."
        ),
        (
            "Runtimes vary by machine; use the JSON artifact for regression tracking "
            "rather than treating one run as a universal score."
        ),
        "",
        "## Reproduce",
        "",
...
def main() -> None:
    ...
    parser.add_argument(
        "--markdown",
        type=Path,
        default=Path("benchmark-results/leaderboard.md"),
    )
    parser.add_argument(
        "--json",
        type=Path,
